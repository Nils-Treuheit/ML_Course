...
def plotNaiveBayes(mc):
    ...
    (xASPlot,xBSPlot,xATPlot,xBTPlot) = ([] for _ in range(4))
    (yASPlot,yBSPlot,yATPlot,yBTPlot) = ([] for _ in range(4))
    ...
    axs[1].set_title(('Navie Bayes (Error='+str(mc)+
    ...
    for x in InstanceList:
        ...
        for i in range(c):
            ...
            if NB[i]>NB[classifier]:
                classifier = i
        pPlot.append(ClassifierList[classifier])
